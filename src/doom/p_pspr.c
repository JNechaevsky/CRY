...
// [Julia] Boolean for Chainsaw attack types
...
{		
    ...
    // [Julia] DON'T use exact Jaguar weapon bobbing formula here,
    // it not very smooth in uncapped framerate mode.
    angle = (128*leveltime)&FINEMASK;
    psp->sx = FRACUNIT + FixedMul (player->bob, finecosine[angle]);
    angle &= FINEANGLES/2-1;
    psp->sy = WEAPONTOP + FixedMul (player->bob, finesine[angle]);

    // [Julia] Jaguar weapon bobbing formula.
    // Note: FRACUNIT is actually WEAPONX.
    // angle = (128*leveltime)&(FINEANGLES-1);
    // psp->sx = FRACUNIT + (player->bob>>16) * finecosine[angle];
    // angle &= FINEANGLES/2-1;
...
void A_Punch (mobj_t *mobj, player_t *player, pspdef_t *psp)
{
    ...
    // [Julia] Use exact Jaguar damage formula
...
void A_Saw (mobj_t *mobj, player_t *player, pspdef_t *psp)
{
    ...
    // [Julia] Chainsaw is working on air
    ...
    // [Julia] Chainsaw is working on someone
...
void A_FireCGun (mobj_t *mobj, player_t *player, pspdef_t *psp)
{
    ...
    // [Julia] Fix Chaingun was making two 
    // figing sound with one remaining bullet.
