...
int detailLevel; // [Julia] & [crispy] Необходимо для WiggleFix
...
void P_LoadSegs (int lump)
{
    ...
    for (i=0 ; i<numsegs ; i++, li++, ml++)
    {
        ...
        // [crispy] recalculate
        li->offset = GetOffset(li->v1, ((ml->side ^ 
                              (flip_levels_cmdline) ? ldef->v2 : ldef->v1)));
            ...
            do
            {
                ...
                if (!v->moved)
                {
                    ...
                    if (v != l->v1 && v != l->v2)
                    {
                        ...
                        v->px = (fixed_t)((dx2 * x0 + dy2 * x1 
                                         + dxy * (y0 - y1)) / s);
                        v->py = (fixed_t)((dy2 * y0 + dx2 * y1 
                                         + dxy * (x0 - x1)) / s);
                    }
                }
