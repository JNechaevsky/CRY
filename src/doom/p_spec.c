...
void P_UpdateSpecials (void)
{
    ...
    // [Julia] Small optimization from Jaguar code: 'gametic&3' condition.
    if (!(gametic&3))
    {
        for (anim = anims ; anim < lastanim ; anim++)
        {
            for (i=anim->basepic ; i<anim->basepic+anim->numpics ; i++)
            {
                ...
                else if (anim->speed == 8 && swirling_liquids)
                {
                    flattranslation[i] = -1;
                }
                else
    ...
    if (buttonlist[i].btimer)
    {
        ...
        if (!buttonlist[i].btimer)
        {
            ...
            // [crispy] & [Julia] Logically proper sound behavior.
            // Do not play second "sfx_swtchn" on two-sided linedefs that 
            // attached to special sectors, and always play second sound 
            // on single-sided linedefs.
...
void R_InterpolateTextureOffsets()
{
    ...
    if (uncapped_fps && !paused && (!menuactive || demoplayback))
    frac = fractionaltic;
	else
    frac = FRACUNIT;
...
int EV_DoDonut (line_t *line)
{
    ...
    // [Julia] Use exact Jaguar code for Donut handling, slightly reformatted
